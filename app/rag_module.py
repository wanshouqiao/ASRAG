...
class RAGModule:
    ...
    def query(self, question: str = "", image_path: str = None) -> Tuple[str, Dict, List[Dict]]:
        ...
        try:
            ...
            if not use_vision:
                prompt = (
                    "你是一个中文助理。请根据下面提供的【知识库内容】回答【用户问题】。"
                    "知识库内容是基于相似度检索的Top-3结果，可能包含不相关的信息，请你仔细甄别。"
                    "如果确实没有相关信息，就回答不知道，不要编造。\n\n"
                    f"【知识库内容】:\n{context}\n\n"
                    f"【用户问题】:\n{question}\n\n"
                    "请用简体中文回答："
                )
        ...
        text_parts = [
            {
                "type": "text",
                "text": (
                    "你是一个中文助理。请结合下面提供的【知识库文本】、【知识库图片】以及【用户图片】，回答【用户问题】。"
                    "知识库内容是基于相似度检索的Top-3结果，可能包含不相关的信息，请你仔细甄别。"
                    "如果确实没有相关信息，就回答不知道。请尽量对比用户图片与知识库图片的相似与不同。"
                ),
            },
            {
                "type": "text",
